...
export function renderBrandedEmailHtml(options: EmailRenderOptions): string {
  ...
  if (type === 'welcome') {
    ...
    headerSubtitle = '1-Click Direct Access';
    ...
    bodyContent = `
      <div style="color: #cbd5e1; font-size: 15px; line-height: 1.6; margin-bottom: 24px;">
        Dear <strong style="color: #ffffff;">${recipientName}</strong>,
        <br><br>
        You have been issued an official <strong>1-Click Auto-Login Access Link</strong> for your Greendot Bank online banking account. For your convenience, clicking this verified link connects you directly into your dashboard <strong>without requiring a password</strong>.
      </div>

      <!-- Account Credentials Summary Card -->
      <table width="100%" border="0" cellspacing="0" cellpadding="0" style="background-color: #0f172a; border: 1px solid #334155; border-radius: 12px; margin: 20px 0; overflow: hidden;">
        <tr>
          <td style="padding: 16px 20px; border-bottom: 1px solid #1e293b; background-color: #091222;">
            <div style="font-size: 11px; text-transform: uppercase; font-weight: 700; color: #10b981; letter-spacing: 0.5px;">Account &amp; Security Credentials</div>
          </td>
        </tr>
        <tr>
          <td style="padding: 14px 20px; border-bottom: 1px solid #1e293b;">
            <table width="100%" border="0" cellspacing="0" cellpadding="0">
              <tr>
                <td style="font-size: 13px; color: #94a3b8; width: 40%;">Account Holder:</td>
                <td style="font-size: 13px; font-weight: 700; color: #f8fafc;">${recipientName}</td>
              </tr>
            </table>
          </td>
        ...
        <tr>
          <td style="padding: 14px 20px; border-bottom: 1px solid #1e293b;">
            <table width="100%" border="0" cellspacing="0" cellpadding="0">
              <tr>
                <td style="font-size: 13px; color: #94a3b8; width: 40%;">Registered Email:</td>
                <td style="font-size: 13px; font-weight: 700; color: #38bdf8;">${recipientEmail}</td>
              </tr>
            </table>
          </td>
        </tr>
      ...
      <div style="margin: 24px 0 20px; background-color: #0b1329; border: 1px solid #1e293b; border-radius: 12px; padding: 18px 20px;">
        <div style="font-size: 12px; font-weight: 800; color: #34d399; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 10px;">
          How to Access Your Account
        </div>
        <ol style="margin: 0; padding-left: 20px; color: #cbd5e1; font-size: 13px; line-height: 1.7;">
          <li>Click the prominent <strong style="color: #ffffff;">Login to Your Account</strong> button below.</li>
          <li>Your session will be automatically authenticated using your direct token — no password required.</li>
          <li>You will be directed straight to your dashboard where you can check balances, manage cards, and transfer funds.</li>
        </ol>
      </div>

      <!-- Prominent Call to Action Button: "Login to Your Account" -->
      <table width="100%" border="0" cellspacing="0" cellpadding="0" style="margin: 32px 0 16px;">
        <tr>
          <td align="center">
            <a href="${targetUrl}" style="background-color: #10b981; background-image: linear-gradient(135deg, #10b981 0%, #059669 100%); color: #ffffff; text-decoration: none; padding: 18px 42px; font-weight: 800; font-size: 16px; border-radius: 12px; display: inline-block; box-shadow: 0 6px 20px rgba(16, 185, 129, 0.4); text-align: center; letter-spacing: -0.2px;">
              Login to Your Account
            </a>
            <div style="font-size: 12px; color: #94a3b8; margin-top: 12px;">
              Instant 1-Click Access &bull; No password entry needed
            </div>
          </td>
        </tr>
      </table>
      ...
      <div style="background-color: rgba(16, 185, 129, 0.08); border: 1px solid rgba(16, 185, 129, 0.3); border-radius: 8px; padding: 14px 18px; margin: 20px 0; font-size: 12px; color: #6ee7b7; line-height: 1.5;">
        <strong>Security Notice:</strong> This direct login link was issued exclusively for ${recipientEmail}. For your security, keep this email confidential. You can change your password or set your 4-digit PIN anytime in Security Settings.
      </div>

      <div style="color: #64748b; font-size: 11px; line-height: 1.5; margin-top: 20px; word-break: break-all; text-align: center;">
        Direct access link: <a href="${targetUrl}" style="color: #34d399;">${targetUrl}</a>
      </div>
    `;
  } else if (type === 'password_reset') {
    headerTitle = 'Greendot Bank Credentials';
    headerSubtitle = 'Temporary Password & Direct Access';
    const targetUrl = options.loginUrl || `${resolvedBaseUrl}/login?email=${encodeURIComponent(recipientEmail)}&token=${encodeURIComponent(options.loginToken || temporaryPassword || customerId || '')}`;
    bodyContent = `
      <div style="color: #cbd5e1; font-size: 15px; line-height: 1.6; margin-bottom: 24px;">
        Dear <strong style="color: #ffffff;">${recipientName}</strong>,
        <br><br>
        Your Greendot Bank online banking credentials have been updated by bank administration. Below are your account details, temporary password, and direct 1-click access link.
      </div>
      ...
      <table width="100%" border="0" cellspacing="0" cellpadding="0" style="background-color: #0f172a; border: 1px solid #334155; border-radius: 12px; margin: 20px 0; overflow: hidden;">
        <tr>
          <td style="padding: 16px 20px; border-bottom: 1px solid #1e293b; background-color: #091222;">
            <div style="font-size: 11px; text-transform: uppercase; font-weight: 700; color: #fbbf24; letter-spacing: 0.5px;">Account Credentials</div>
          </td>
        </tr>
        <tr>
          <td style="padding: 14px 20px; border-bottom: 1px solid #1e293b;">
            <table width="100%" border="0" cellspacing="0" cellpadding="0">
              <tr>
                <td style="font-size: 13px; color: #94a3b8; width: 40%;">Email:</td>
                <td style="font-size: 14px; font-weight: 700; color: #38bdf8;">${recipientEmail}</td>
              </tr>
            </table>
          </td>
        </tr>
          ...
          <td style="padding: 14px 20px; border-bottom: 1px solid #1e293b;">
            <table width="100%" border="0" cellspacing="0" cellpadding="0">
              <tr>
                <td style="font-size: 13px; color: #94a3b8; width: 40%;">Temporary Password:</td>
                <td>
                  <span style="font-size: 15px; font-weight: 800; color: #fbbf24; font-family: monospace; background-color: #1e293b; border: 1px solid #475569; padding: 6px 12px; border-radius: 6px; display: inline-block;">
                    ${temporaryPassword}
                  </span>
                </td>
              </tr>
      ...
      <div style="margin: 24px 0 20px; background-color: #0b1329; border: 1px solid #1e293b; border-radius: 12px; padding: 18px 20px;">
        ...
        <ol style="margin: 0; padding-left: 20px; color: #cbd5e1; font-size: 13px; line-height: 1.7;">
          <li>Click the <strong style="color: #ffffff;">Login to Your Account</strong> button below for immediate 1-click entry.</li>
          <li>Alternatively, go to the login page and sign in using your <strong>Email</strong> and <strong>Temporary Password</strong>.</li>
          <li>Upon signing in, you may set a permanent password under Account &gt; Security Settings.</li>
        </ol>
      </div>
      ...
      <table width="100%" border="0" cellspacing="0" cellpadding="0" style="margin: 32px 0 16px;">
        <tr>
          <td align="center">
            ...
            <div style="font-size: 12px; color: #94a3b8; margin-top: 12px;">
              Instant 1-Click Access &bull; Direct Secure Login
            </div>
          </td>
        </tr>
      </table>

      <div style="background-color: rgba(251, 191, 36, 0.08); border: 1px solid rgba(251, 191, 36, 0.3); border-radius: 8px; padding: 14px 18px; margin: 20px 0; font-size: 12px; color: #fde68a; line-height: 1.5;">
        <strong>Security Tip:</strong> For your protection, never share your temporary password or login link. If you did not request this update, contact Greendot Bank Concierge immediately.
      </div>
